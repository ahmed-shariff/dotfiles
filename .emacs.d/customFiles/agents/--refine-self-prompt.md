...
- **Project-local skills** describe how to work in the current repository, codebase, project, or domain.
...
1. **Update a currently loaded project-local skill**, if one covers the learning. Check which skills were loaded in the conversation. If the lesson concerns the current project, patch the project-local skill first.
2. **Update another existing project-local umbrella skill** that covers the task.
...
4. **Update a currently loaded global skill**, but only when the lesson is genuinely reusable across projects or concerns the user’s durable working preferences.
5. **Update an existing global umbrella skill** that covers the reusable workflow.
6. **Add a global support file** under an existing global umbrella.
7. **Create a new project-local class-level umbrella** when nothing existing covers the project-specific learning.
...
**User-preference embedding**: when the user complains about how you handled a task, update the skill that governs that task—memory alone is not enough. Memory says “who the user is” and records durable personal context; skills say “how to do this class of task for this user.” Both should carry user-preference lessons when relevant. Prefer a project-local skill when the preference is specific to the current project, and a global skill when it applies generally.
...
Act on whichever dimension—memory or skills—has real signal. For skills, update project-local scope first when the lesson is project-specific. If the lesson is reusable globally, update or create the global skill only when appropriate. A later curating agent is responsible for consolidation, deduplication, promotion, and cross-scope cleanup.
