...
(cl-defstruct (gptel-openai-assistant--threads
               (:constructor gptel-openai--make-assistant-threads)
               (:copier nil)
               (:include gptel-openai)))

(cl-defmethod gptel--parse-response ((backend gptel-openai-assistant--threads) response _info)
  ;; (pcase (gptel-openai-assistant-step ))
  (plist-get response :id))

(cl-defmethod gptel--request-data ((_backend gptel-openai-assistant--threads) prompts)
  nil)

(cl-defstruct (gptel-openai-assistant--messages
               (:constructor gptel-openai--make-assistant-messages)
               (:copier nil)
               (:include gptel-openai)))

(cl-defmethod gptel--request-data ((_backend gptel-openai-assistant--messages) prompts)
  (list :role "user"
        :content `[(:type "text" :text [,@prompts])]))

(cl-defstruct (gptel-openai-assistant--runs
               (:constructor gptel-openai--make-assistant-runs)
               (:copier nil)
               (:include gptel-openai)))

(cl-defmethod gptel-curl--parse-stream ((_backend gptel-openai-assistant--runs) _info)
  (let* ((content-strs))
    (condition-case nil
...
(cl-defmethod gptel--parse-response ((_backend gptel-openai-assistant--runs) response _info)
...
(cl-defmethod gptel--request-data ((_backend gptel-openai-assistant--runs) _prompts)
  (let ((prompts-plist
...
(setq gptel-openai-assistant-backend--start-thread (gptel-openai--make-assistant-threads
                                                    :name "gptel-openai-assistant--threads"
                                                    :host "api.openai.com"
                                                    :key 'gptel-api-key
...
(setq gptel-openai-assistant-backend--add-messages (gptel-openai--make-assistant-messages
                                                    :name "gptel-openai-assistant--messages"
                                                    :host "api.openai.com"
                                                    :key 'gptel-api-key
                                                    :header (lambda () (when-let (key (gptel--get-api-key))
                                                                         `(("Authorization" . ,(concat "Bearer " key))
                                                    ...
                                                    :url (lambda ()
                                                           (if gptel-openai-assistant-thread-id
                                                             ...
                                                             (user-error "No thread in current buffer!")))))

(setq gptel-openai-assistant-backend--run (gptel-openai--make-assistant-runs
                                           :name "gptel-openai-assistant--run"
                                           :host "api.openai.com"
                                           :key 'gptel-api-key
                                           :header (lambda () (when-let (key (gptel--get-api-key))
                                                                `(("Authorization" . ,(concat "Bearer " key))
...
(defun gptel-openai-assistant-start-thread ()
  (let ((gptel-backend gptel-openai-assistant-backend--start-thread))
    (funcall (if gptel-use-curl
                 #'gptel-curl-get-response #'gptel--url-get-response)
             `(:data ,(gptel--request-data gptel-openai-assistant-backend--start-thread nil)
                     :buffer ,(current-buffer))
             (lambda (response info)
               (with-current-buffer (plist-get info :buffer)
                 (setq-local gptel-openai-assistant-thread-id response))))))

(defun gptel-openai-assistant-add-messages ()
  (let ((gptel-backend gptel-openai-assistant-backend--add-messages))
    (gptel-request nil :stream nil)))

(defun gptel-openai-assistant-run ()
  (let ((gptel-backend gptel-openai-assistant-backend--run))
...
(defun gptel-openai-assistant-send ()
  ...
  (gptel-openai-assistant-add-messages)
