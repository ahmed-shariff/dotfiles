# from https://github.com/jeremyFreeAgent/oh-my-zsh-powerline-theme
...
# else
    if [ "$POWERLINE_SHOW_GIT_ON_RIGHT" = "" ]; then
        if [ "$POWERLINE_HIDE_GIT_PROMPT_STATUS" = "" ]; then
            POWERLINE_GIT_INFO_LEFT=" %F{$COLOR_2_BG}%K{$COLOR_3_BG}"$'\ue0b0'"%F{$COLOR_3_BG}%F{$COLOR_3_FG}%K{$COLOR_3_BG}"$'$(git_prompt_info)$(git_prompt_status)%F{$COLOR_3_BG}'
        else
            POWERLINE_GIT_INFO_LEFT=" %F{$COLOR_2_BG}%K{$COLOR_3_BG}"$'\ue0b0'"%F{$COLOR_3_BG}%F{$COLOR_3_FG}%K{$COLOR_3_BG}"$'$(git_prompt_info)%F{$COLOR_3_BG}'
        fi
        POWERLINE_GIT_INFO_RIGHT=""
    else
        POWERLINE_GIT_INFO_LEFT=""
...
PROMPT="%F{$COLOR_1_FG}%K{$COLOR_1_BG} $POWERLINE_USER_NAME %k%f%F{$COLOR_1_BG}%K{$COLOR_2_BG}"$'\ue0b0'"%k%f%F{255}%K{$COLOR_2_BG}"$POWERLINE_PATH""$POWERLINE_GIT_INFO_LEFT" %k"$'\ue0b0'"%f "
...
if [ "$POWERLINE_DISABLE_RPROMPT" = "" ]; then
    if [ "$POWERLINE_RIGHT_A" = "" ]; then
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{$COLOR_3_BG}"$'\ue0b2'"%k%F{$COLOR_3_FG}%K{$COLOR_3_BG} $POWERLINE_RIGHT_B %f%k"
    elif [ "$POWERLINE_RIGHT_B" = "" ]; then
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{$COLOR_3_BG}"$'\ue0b2'"%k%F{$POWERLINE_RIGHT_A_COLOR_FRONT}%K{$POWERLINE_RIGHT_A_COLOR_BACK} $POWERLINE_RIGHT_A %f%k"
    else
        RPROMPT="$POWERLINE_GIT_INFO_RIGHT%F{$COLOR_3_BG}"$'\ue0b2'"%k%F{$COLOR_3_FG}%K{$COLOR_3_BG} $POWERLINE_RIGHT_B %f%F{$POWERLINE_RIGHT_A_COLOR_BACK}"$'\ue0b2'"%f%k%K{$POWERLINE_RIGHT_A_COLOR_BACK}%F{$POWERLINE_RIGHT_A_COLOR_FRONT} $POWERLINE_RIGHT_A %f%k"
    fi
fi

if [ $(ps -p $(echo $PPID) -o cmd -h) = 'emacs' ]; then
   PROMPT=%n$'\ue0b0'" $POWERLINE_PATH "$'\ue0b0'" "
   RPROMPT=""
fi
